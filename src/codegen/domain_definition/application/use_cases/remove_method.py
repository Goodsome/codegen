    ...
    def execute(self, cmd: RemoveMethodCommand) -> RemoveMethodResult:
        ...
        match (cmd.element_type, cmd.method_kind):
            case (ElementType.AGGREGATE, MethodKind.BEHAVIOR):
                context.domain.get_aggregate(cmd.element_name).remove_behavior(method_name)
            case (ElementType.ENTITY, MethodKind.BEHAVIOR):
                context.domain.get_entity(cmd.element_name).remove_behavior(method_name)
            case (ElementType.VALUE_OBJECT, MethodKind.BEHAVIOR):
                context.domain.get_value_object(cmd.element_name).remove_behavior(method_name)
            case (ElementType.DOMAIN_SERVICE, MethodKind.OPERATION):
                context.domain.get_service(cmd.element_name).remove_operation(method_name)
            case (ElementType.APP_SERVICE, MethodKind.OPERATION):
                context.application.get_service(cmd.element_name).remove_operation(method_name)
            ...
            case (ElementType.APP_PORT, MethodKind.OPERATION):
                context.application.get_port(cmd.element_name).remove_operation(method_name)
            case (ElementType.IMPLEMENTATION, MethodKind.PRIVATE):
                context.infrastructure.get_implementation(cmd.element_name).remove_private_method(method_name)
            case _:
                raise ValueError(
