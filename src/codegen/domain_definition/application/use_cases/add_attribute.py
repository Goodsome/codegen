...
class AddAttributeCommand(BaseModel):
    """Command to add an attribute, dependency, input, or output to an element."""
...
class AddAttribute:
    ...
    def execute(self, cmd: AddAttributeCommand) -> AddAttributeResult:
        ...
        match (cmd.element_type, cmd.attribute_kind):
            ...
            case (ElementType.VALUE_OBJECT, AttributeKind.ATTRIBUTE):
                context.domain.get_value_object(cmd.element_name).add_attribute(attribute)
            ...
            case (ElementType.APP_SERVICE, AttributeKind.DEPENDENCY):
                context.application.get_service(cmd.element_name).add_dependency(attribute)
            case (ElementType.USE_CASE, AttributeKind.DEPENDENCY):
                context.application.get_use_case(cmd.element_name).add_dependency(attribute)
            ...
            case (ElementType.IMPLEMENTATION, AttributeKind.ATTRIBUTE):
                context.infrastructure.get_implementation(cmd.element_name).add_attribute(attribute)
            case _:
                raise ValueError(
