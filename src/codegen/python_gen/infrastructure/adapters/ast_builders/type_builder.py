...
def build_type_annotation(type_spec: TypeAnnotationSpec) -> ast.expr:
    ...
    if type_spec.args:
        args_nodes = []
        for arg in type_spec.args:
            if isinstance(arg, TypeAnnotationSpec):
                args_nodes.append(build_type_annotation(arg))
            elif isinstance(arg, AssignmentSpec):
                # Handle AssignmentSpec (e.g., typer.Argument(...))
                args_nodes.append(build_assignment_expr(arg))
