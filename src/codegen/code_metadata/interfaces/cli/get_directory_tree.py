...
def _add_children(parent: Tree, node_tree: NodeTree) -> None:
    for child in node_tree.children:
        ...
        branch = parent.add(f"{icon} [bold]{child.node.name}[/bold]  [dim]{child.node.fqn}[/dim]")
        _add_children(branch, child)


def get_directory_tree(
    fqn_prefix: Annotated[str, typer.Argument(help="FQN prefix of the root directory node")],
) -> None:
