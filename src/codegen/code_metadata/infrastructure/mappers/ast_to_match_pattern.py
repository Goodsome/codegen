...
class AstToMatchPattern:
    ...
    def from_match_sequence(node: ast.MatchSequence) -> MatchSequence:
        return MatchSequence(patterns=[AstToMatchPattern.to_match_pattern(p) for p in node.patterns])
    ...
    def from_match_class(node: ast.MatchClass) -> MatchClass:
        return MatchClass(
            ...
            kwd_patterns=[AstToMatchPattern.to_match_pattern(p) for p in node.kwd_patterns],
        )
    ...
    def from_match_as(node: ast.MatchAs) -> MatchAs:
        return MatchAs(
            pattern=AstToMatchPattern.to_match_pattern(node.pattern) if node.pattern else None,
            name=node.name,
        )

    @staticmethod
    def from_match_or(node: ast.MatchOr) -> MatchOr:
        return MatchOr(patterns=[AstToMatchPattern.to_match_pattern(p) for p in node.patterns])
