...
class ModuleMapper:
    ...
    @classmethod
    def to_domain(cls, orm_model: ModuleModel) -> FileModule | DirectoryModule | ExternalModule:
    ...
    def _to_file_module(cls, orm_model: ModuleModel) -> FileModule:
        ...
        return FileModule(
            ...
            path=orm_model.path,
            components=[
                ComponentV2Mapper.to_domain(c) for c in orm_model.components
            ],
            dependencies=[
                ModuleDependency.model_validate(d)
                for d in orm_model.dependencies
            ],
            dir_module_id=(
    ...
    def _to_directory_module(cls, orm_model: ModuleModel) -> DirectoryModule:
        ...
        return DirectoryModule(
            ...
            sub_module_ids=[
                ModuleId.reconstitute(s) for s in orm_model.sub_module_ids
            ],
            dir_module_id=(
                ModuleId.reconstitute(orm_model.dir_module_id)
                if orm_model.dir_module_id
    ...
    def _file_to_orm(cls, domain_entity: FileModule) -> FileModuleModel:
        return FileModuleModel(
            ...
            components=[
                ComponentV2Mapper.to_orm(c)
                for c in domain_entity.components
            ],
        )
    ...
    def _directory_to_orm(cls, domain_entity: DirectoryModule) -> DirectoryModuleModel:
        return DirectoryModuleModel(
            ...
            sub_module_ids=[
                str(s) for s in domain_entity.sub_module_ids
            ],
            dir_module_id=(
                domain_entity.dir_module_id.value
                if domain_entity.dir_module_id
