"""FunctionNode 的三向转换（ORM ↔ Domain ↔ DTO）。"""
...
from codegen.code_metadata.infrastructure.orm_models import FunctionNodeModel


class FunctionNodeMapper:
    """FunctionNode 专属的三向 Mapper。"""

    @classmethod
    def to_dto(cls, orm_model: FunctionNodeModel) -> FunctionNode:
        decorator_list = [
        ...
        body = [
            ast_stmt_adapter.validate_python(stmt)
            for stmt in orm_model.body
        ]
        return FunctionNode(
            fqn=orm_model.fqn,
            name=orm_model.name,
    ...
    @classmethod
    def to_properties(cls, dto: FunctionNode) -> dict[str, Any]:
        decorator_list = [
        ...
        body = [
            ast_stmt_adapter.dump_python(stmt, mode="json")
            for stmt in dto.body
        ]
        return {
            "decorator_list": decorator_list,
            "returns": returns,
