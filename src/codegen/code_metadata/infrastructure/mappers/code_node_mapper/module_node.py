...
class ModuleNodeMapper:
    ...
    def to_dto(cls, orm_model: CodeNodeModel) -> ModuleNode:
        ...
        exprs = [
            ast_expr_adapter.validate_python(expr)
            for expr in orm_model.exprs
        ]
        return ModuleNode(
            fqn=orm_model.fqn,
            name=orm_model.name,
    ...
    def to_properties(cls, dto: ModuleNode) -> dict[str, Any]:
        exprs = [
            ast_expr_adapter.dump_python(expr, mode="json")
            for expr in dto.exprs
        ]
        return {
            "is_package": dto.is_package,
            "exprs": exprs,
        }
