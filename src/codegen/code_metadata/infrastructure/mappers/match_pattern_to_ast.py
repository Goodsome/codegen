...
class MatchPatternToAst:
    ...
    def to_node(pattern: MatchPattern) -> ast.pattern:
        match pattern:
            ...
            case _:
                raise NotImplementedError(f"Unsupported MatchPattern type: {type(pattern)}, {pattern=}")

    @staticmethod
    def from_match_value(pattern: MatchValue) -> ast.MatchValue:
        return ast.MatchValue(value=ast.parse(pattern.value, mode="eval").body if pattern.value else None)
    ...
    def from_match_sequence(pattern: MatchSequence) -> ast.MatchSequence:
        return ast.MatchSequence(patterns=[MatchPatternToAst.to_node(p) for p in pattern.patterns])
    ...
    def from_match_as(pattern: MatchAs) -> ast.MatchAs:
        return ast.MatchAs(
            pattern=MatchPatternToAst.to_node(pattern.pattern) if pattern.pattern else None,
            name=pattern.name,
        )

    @staticmethod
    def from_match_or(pattern: MatchOr) -> ast.MatchOr:
        return ast.MatchOr(patterns=[MatchPatternToAst.to_node(p) for p in pattern.patterns])
