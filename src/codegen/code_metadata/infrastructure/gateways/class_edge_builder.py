...
class ClassEdgeBuilder:
    ...
    def _parse_stmt(self, stmt: AstStmt):
        match stmt:
            case AstFunctionDef():
                self._parse_function_def(stmt)
            case _:
                pass

    def _parse_function_def(self, func_def: AstFunctionDef):
    ...
    def _build_function_returns_edge(
        ...
        match returns:
            ...
            case AstAttribute():
                pass
            case _:
                raise ValueError(f"Unsupported returns type: {returns}")
    ...
    def _visit_stmt(self, stmt: AstStmt):
        match stmt:
            ...
            case AstPass():
                pass
            case _:
                raise ValueError(f"Unsupported: {stmt=}\n {self.current_node=}")
