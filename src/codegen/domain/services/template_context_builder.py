...
class TemplateContextBuilder:
    ...
    def build_registry(self, blueprint: Blueprint):
        ...
        for ctx in blueprint.contexts:
            for agg in ctx.aggregates:
                self.registry[agg.name] = f"from codegen.domain.aggregates.{self.naming.to_snake(agg.name)} import {agg.name}"
            for vo in ctx.value_objects:
                self.registry[vo.name] = f"from codegen.domain.value_objects.{self.naming.to_snake(vo.name)} import {vo.name}"
            for port in ctx.ports:
                self.registry[port.name] = f"from codegen.domain.ports.{self.naming.to_snake(port.name)} import {port.name}"
            for svc in ctx.services:
                self.registry[svc.name] = f"from codegen.domain.services.{self.naming.to_snake(svc.name)} import {svc.name}"
    ...
    def resolve_imports(self, types_used: Set[str], current_name: str, force_dataclass: bool = False) -> List[str]:
        imports = []
    ...
    def build_context(self, target: Any) -> Dict[str, Any]:
        ...
        if hasattr(target, "attributes"):
            types_used.update(self.extract_types([a.name if hasattr(a, "type") else "" for a in target.attributes]))
            # Wait, attributes in domain model are objects.
            # a.type should be the type string.
            types_used.update(self.extract_types([getattr(a, "type", "") for a in target.attributes]))

        if hasattr(target, "operations"):
            for op in target.operations:
                types_used.update(self.extract_types([getattr(i, "type", "") for i in op.inputs]))
        ...
        if hasattr(target, "command") and hasattr(target, "result"):
            ...
            types_used.update(self.extract_types([getattr(a, "type", "") for a in target.command.attributes]))
            types_used.update(self.extract_types([getattr(a, "type", "") for a in target.result.attributes]))
            if getattr(target, "depends_on_services", []) or getattr(target, "depends_on_ports", []):
                types_used.add("Any")
        ...
        ctx["imports"] = self.resolve_imports(types_used, ctx.get("name", ""), force_dataclass)
